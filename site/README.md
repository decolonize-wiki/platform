...
- **Environment variables**: set `DATA_TARBALL_URL`, `METHODOLOGY_TARBALL_URL`,
  `DATA_DIR=.data`, `METHODOLOGY_DIR=.methodology`, plus `RESEND_API_KEY`.
  (Contacts are account-wide in Resend's current API — no audience ID needed.)
...
The `/api/subscribe` route creates a contact in Resend (contacts are
account-wide in Resend's current API; segments/topics can partition them later).
API-created contacts do not receive a confirmation email, so the form does not
promise one; a real double opt-in flow (confirmation email + confirm link) is a
future enhancement.
